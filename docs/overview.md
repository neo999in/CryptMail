...
CipherMail layers end-to-end encryption on top. Your contacts, your address,
your history — unchanged. What changes is that messages between CipherMail users
...
2. **Encrypt on send.** Mail sent through CipherMail to another CipherMail user
...
4. **Seamless inside the app.** In CipherMail the same message renders as normal
...
- *As a privacy-conscious user*, I sign in with my Gmail account and immediately
  keep using my inbox; new mail I send to other CipherMail users is encrypted
  without me doing anything.
- *As a recipient*, I get an email that looks encrypted in Gmail; I open
  CipherMail and read it normally.
...
- *As a multi-device user*, I add CipherMail on my laptop and my existing
...
Any non-CipherMail client renders this as-is (or as an unopenable attachment).
...
- A user can OAuth into Gmail, send an encrypted message to another CipherMail
