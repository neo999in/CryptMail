...
| Device transfer — move key, Key Manager bank and archive to a new phone ([design](superpowers/specs/2026-09-19-per-email-keys-design.md#device-transfer); per-email keys themselves were removed 2026-09-22) | [`core/src/transfer.rs`](../core/src/transfer.rs), [`state/identity.ts`](../app/src/state/identity.ts) | `Settings → Move to a new phone`; restore field in Setup and Recovery | 8 Rust e2e + 16 |
