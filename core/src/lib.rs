...
impl Core {
    ...
    /// Seal this phone's identity, key bank and `archive` for another phone
    /// under `code`, and hand the bank over: from now on this phone reads but
    /// does not issue quantum keys.
    ...
    /// `{ handedOverAt }`: when this phone's key bank went to another, in Unix
    /// seconds, or null.
    pub fn transfer_status(&self, passphrase: &str) -> Result<String> {
        let at = match identity::stored_email(&self.dir)? {
            Some(email) => km::KeyManager::for_account(&self.dir, passphrase, &email)?.handed_over_at()?,
            None => None,
        };
        Ok(serde_json::json!({ "handedOverAt": at }).to_string())
    }

    /// Take the key bank back after a transfer that was never used. Only safe
    /// if the other phone never sent with a quantum key; the app says so first.
    pub fn resume_transfer(&self, passphrase: &str) -> Result<()> {
