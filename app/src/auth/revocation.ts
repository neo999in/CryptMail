 ...
 *
 * Split out of `googleAuth.ts` so it can be tested without pulling in the
 * Play-services sign-in module, which is native. This is the judgement call in
 * the whole re-auth path, so it is the part that most needs tests.
 */
 ...
 * because token errors surface inconsistently — the sign-in module throws a
 * `NativeModuleError` carrying a `code`, while a failed token call can arrive
 * as a plain `Error` whose message carries the JSON body.
 */
