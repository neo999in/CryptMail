/**
 * The old in-memory demo mailbox, kept **only** as a test fixture.
 *
 * It used to be a real mail path: `state/session.ts` built one whenever the
 * signed-in provider was `demo`, which is how an unconfigured app came to show a
 * working-looking inbox full of invented mail. That path is gone — Gmail is the
 * only mailbox — so nothing in `src/` outside this directory imports this file,
 * and it cannot re-enter the product without someone deliberately wiring it back.
 *
 * It survives because of the three filter fixtures at the bottom. They are the
 * standing regression test for the spam engine in both directions, and
 * `demo-legit-security` in particular is the shape of mail a keyword filter ruins
 * (see docs/SPAM_PHISHING_DETECTION.md). Reproducing that coverage from scratch
 * would mean rebuilding the same messages by hand, so the messages stay and only
 * their reachability changed.
 *
 * Lives in `__tests__/` and is not named `*-test.ts`, so jest's `testMatch`
 * ignores it as a suite while `demoMail-test.ts` imports it — the same
 * arrangement as `pgp/__tests__/fixtures.ts`.
 *
 ...
 *
 * So with a native core this mailbox serves only what stays true: the plaintext
 * fixtures, plus a message explaining the absence.
 *
 * ## The filter fixtures
 *
 ...
 * important one — it is the shape of mail a keyword filter ruins, so a regression
 * in the false-positive direction fails a test rather than going unnoticed. Every
 * URL in them is on a `.example`/`.invalid` domain, which cannot resolve; nothing
 * here is ever fetched in any case.
 */
...
export async function createDemoMailClient(
  ...
  return {
    ...
    async send(rfc822) {
      ...
      // ciphertext included. That is the point of the fixture.
...
async function seed(address: string, includeDemoCiphertext: boolean): Promise<Stored[]> {
  ...
  if (!includeDemoCiphertext) {
    // Say plainly why the mailbox is thinner than the design shows, rather than
 ...
 *
 * `demo-legit-security` is deliberately the hardest case here. It says
...
function toStored(id: string, raw: string, unread: boolean, threadId?: string, date?: string): Stored {
  ...
  return {
    ...
    summary: {
      ...
      // it does not, because absence must never read as failure. That parity is
      // what makes these fixtures meaningful evidence about the live path.
 ...
 *
 * This used to return only the first non-blank line, which for a message opening
 * "Dear valued customer," is four words of salutation. That made the fixtures
 * score plaintext mail on materially less text than the live mailbox would, and it
 * silently disarmed the bulk-mail fixture: `demo-bulk`'s prize-and-payment wording
 * sits on lines two and four, so the row was categorised as though the message said
 * nothing at all. Flattening to a prefix is what makes these fixtures and the Gmail
 * path read the same shape of text.
 */
