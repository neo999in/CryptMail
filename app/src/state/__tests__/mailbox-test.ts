...
const mockSaveSpamState = jest.fn<Promise<void>, [SpamState]>(async () => {});
...
jest.mock('../../store/spamModelStore', () => ({
  ...
  saveSpamState: (state: SpamState) => mockSaveSpamState(state),
...
describe('markSpam', () => {
  ...
  it('persists the mark and the model together, in one record', async () => {
    ...
    const saved = mockSaveSpamState.mock.calls[0][0];
