 ...
 *
 * The record is **per account**, keyed like every other scoped store: what one
 * mailbox learns is not evidence about another's mail, and a removed account
 * must take its model and its marks with it.
 *
...
export async function loadSpamState(account: AccountId): Promise<SpamState> {
  const raw = await loadScopedJson<unknown>(SPAM_STORE_KEY, account, null);
  return normaliseSpamState(raw);
}

export async function saveSpamState(account: AccountId, state: SpamState): Promise<void> {
  await saveScopedJson(SPAM_STORE_KEY, account, { model: state.model, marks: trimMarks(state.marks) });
}
